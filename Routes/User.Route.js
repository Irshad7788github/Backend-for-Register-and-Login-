...
Router.post('/register', async(req,res)=>{
    try {
        ...
        if(userExit){
            ...
            const Auth_Tokan = jwt.sign(data, secratekey )
            res.status(200).json({
                mgs:"Registration succesfull ", Auth_Tokan
            })
...
Router.post('/login', async(req,res)=>{
    try {
        ...
        if(!userExist){
            ...
            const AuthTokan = jwt.sign(data, secratekey)
            res.status(200).json({
                mgs:"Login successfull", AuthTokan
            })
        }
      }
